...
// It validates the go-gen-bpf spec fields:
// - sourceDir (string, optional) - directory containing BPF C source files
...
func handleConfigValidate(
	...
	log.Printf("Validating go-gen-bpf config for spec: %s", input.SpecName)

	output := validateGoGenBpfSpec(input.Spec)
...
// validateGoGenBpfSpec validates the go-gen-bpf spec fields.
func validateGoGenBpfSpec(spec map[string]interface{}) *mcptypes.ConfigValidateOutput {
	...
	// Validate sourceDir (optional, string)
	_, err := mcptypes.ValidateString(spec, "sourceDir")
	if err != nil {
		output.Valid = false
