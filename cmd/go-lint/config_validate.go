...
// handleConfigValidate validates go-lint engine configuration.
// It checks the spec fields:
//   - config (string, optional) - path to golangci-lint config file
//   - args ([]string, optional) - additional arguments to pass to golangci-lint
//
...
// validateSpec validates the go-lint spec fields.
...
func validateSpec(spec map[string]interface{}) *mcptypes.ConfigValidateOutput {
	...
	// Validate config field (optional, must be string if present)
	if _, verr := mcptypes.ValidateString(spec, "config"); verr != nil {
