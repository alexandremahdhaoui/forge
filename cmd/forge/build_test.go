...
func TestBuildIntegration(t *testing.T) {
	...
	expectedBinaries := []string{"forge", "go-build", "container-build", "testenv-kind", "testenv-lcr", "testenv-helm-install", "go-test"}
...
func TestBuildSingleArtifact(t *testing.T) {
	...
	output, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("forge build go-lint failed: %v\nOutput: %s", err, string(output))
	}

	// Verify go-lint binary exists
	if _, err := os.Stat(testBinPath); os.IsNotExist(err) {
		t.Fatal("go-lint binary was not built")
	}

	t.Log("Successfully built single artifact: go-lint")
}
...
func TestBuildWithFormatter(t *testing.T) {
	...
	// The output should contain "Formatting Go code" from go-format
	if !strings.Contains(outputStr, "Formatting Go code") && !strings.Contains(outputStr, "Formatted Go code") {
