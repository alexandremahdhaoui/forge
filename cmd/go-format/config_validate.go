...
// handleConfigValidate validates go-format engine configuration.
// It checks the spec fields:
//   - path (string, optional) - path to format
//
...
// validateSpec validates the go-format spec fields.
...
func validateSpec(spec map[string]interface{}) *mcptypes.ConfigValidateOutput {
	...
	// Validate path field (optional, must be string if present)
	if _, verr := mcptypes.ValidateString(spec, "path"); verr != nil {
