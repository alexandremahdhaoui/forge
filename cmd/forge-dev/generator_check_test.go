...
func TestAGeneratedChartRootWithTheGeneratedHeaderOnLineOneIsAccepted(t *testing.T) {
	for _, path := range []string{"chart/Chart.yaml", "chart/values.yaml"} {
		t.Run(path+" is accepted", func(t *testing.T) {
			dir := t.TempDir()
			writeKindFixture(t, dir, customKindYaml())

			require.NoError(t, generateCell(t, dir, answerFiles(headeredFileWith("#", path))))
			require.FileExists(t, filepath.Join(dir, filepath.FromSlash(path)))
		})
	}
}

func TestAGeneratedChartYamlWithoutTheGeneratedHeaderIsRefusedNamingThePath(t *testing.T) {
	dir := t.TempDir()
	writeKindFixture(t, dir, customKindYaml())

	err := generateCell(t, dir, answerFiles(headerlessFile("chart/Chart.yaml")))

	require.Error(t, err)
	require.Contains(t, err.Error(),
		"the generated root chart/Chart.yaml is missing the generated header on line one")
	require.NoFileExists(t, filepath.Join(dir, "chart", "Chart.yaml"))
}

func TestALowerCaseChartYamlIsRefusedAsNotNamedZzGenerated(t *testing.T) {
	dir := t.TempDir()
	writeKindFixture(t, dir, customKindYaml())

	err := generateCell(t, dir, answerFiles(headeredFileWith("#", "chart/chart.yaml")))

	require.Error(t, err)
	require.Contains(t, err.Error(), "chart/chart.yaml is not named zz_generated")
	require.NoFileExists(t, filepath.Join(dir, "chart", "chart.yaml"))
}

func TestARecordedValuesYamlIsRemovableOnlyWhenItCarriesTheGeneratedHeader(t *testing.T) {
	dir := t.TempDir()
	valuesPath := filepath.Join(dir, "chart", "values.yaml")
	require.NoError(t, os.MkdirAll(filepath.Dir(valuesPath), 0o755))

	require.NoError(t, os.WriteFile(valuesPath, []byte("# Code generated by gui-gen. DO NOT EDIT.\nreplicas: 1\n"), 0o644))
	require.True(t, removableGeneratedPath(dir, "chart/values.yaml"))

	require.NoError(t, os.WriteFile(valuesPath, []byte("replicas: 1\n"), 0o644))
	require.False(t, removableGeneratedPath(dir, "chart/values.yaml"))
}
...
func TestAModuleRootWithoutTheGeneratedHeaderIsRefusedNamingThePath(t *testing.T) {
	...
	require.Error(t, err)
	require.Contains(t, err.Error(),
		"the generated root src/grpc/mod.rs is missing the generated header on line one")
...
func TestARecordedEntryThatEscapesTheEngineDirectoryIsSkippedInsteadOfRemoved(t *testing.T) {
	...
	require.Contains(t, logged.String(),
		"forge-dev: skipped the recorded entry ../../x, it is not removable, "+
			"it must be named zz_generated or be a generated root carrying the generated header, "+
