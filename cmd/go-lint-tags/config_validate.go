...
// handleConfigValidate validates go-lint-tags engine configuration.
// It checks the spec fields:
//   - expectedTags ([]string, optional) - list of expected build tags
//   - rootDir (string, optional) - root directory to scan for test files
//
...
// validateSpec validates the go-lint-tags spec fields.
...
func validateSpec(spec map[string]interface{}) *mcptypes.ConfigValidateOutput {
	...
	// Validate expectedTags field (optional, must be []string if present)
	if _, verr := mcptypes.ValidateStringSlice(spec, "expectedTags"); verr != nil {
