...
// handleConfigValidate validates go-lint-licenses engine configuration.
// It checks the spec fields:
//   - rootDir (string, optional) - root directory to scan for Go files
//   - excludePatterns ([]string, optional) - patterns to exclude from scanning
//
...
// validateSpec validates the go-lint-licenses spec fields.
...
func validateSpec(spec map[string]interface{}) *mcptypes.ConfigValidateOutput {
	...
	// Validate rootDir field (optional, must be string if present)
	if _, verr := mcptypes.ValidateString(spec, "rootDir"); verr != nil {
		errors = append(errors, *verr)
	}

	// Validate excludePatterns field (optional, must be []string if present)
	if _, verr := mcptypes.ValidateStringSlice(spec, "excludePatterns"); verr != nil {
		errors = append(errors, *verr)
	}
